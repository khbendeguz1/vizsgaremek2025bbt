...
$sql = "SELECT * FROM cars WHERE kategoria = '3' AND berelve = '0'";
if ($search) {
    $sql .= " AND marka LIKE '%$search%' AND kategoria = '3'";
}
...
session_start();
$user_id = $_SESSION['user_id']; // Felhasználó ID-je, amit be kell állítani bejelentkezéskor
$user_query = "SELECT nev, email FROM users WHERE id = '$user_id'";
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <a href="berles.php?auto_id=<?= $row['id'] ?>&jogositvany_szam=<?= $row['jogositvany_szam'] ?>&berles_kezdet=<?= date('Y-m-d') ?>&nev=<?= $user['nev'] ?>&email=<?= $user['email'] ?>" class="btn btn-primary">Bérlés</a>
                    </div>
                </div>
            </div>
        </div>
